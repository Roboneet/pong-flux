...
model = load_model(base, value, adv)
...
function save_model(model::nn)
  ...
  @save "models/$(game)/duel_dqn_base" base_wt
  @save "models/$(game)/duel_dqn_val" val_wt
  @save "models/$(game)/duel_dqn_adv" adv_wt

  println("Model saved")
end

function load_model()
  @load "models/$(game)/duel_dqn_base" base_wt
  @load "models/$(game)/duel_dqn_val" val_wt
  @load "models/$(game)/duel_dqn_adv" adv_wt
...
preprocess(I::Array{Any,1}) = UInt8.(reshape(I, 80,80,1))
...
function action(π::PongPolicy, reward, state, action)
  ...
  act_values = model(toFloat.(s))
  π.prev_act = Flux.argmax(act_values)[1]

  return π.prev_act  # returns action max Q-value
end

toFloat(state) = Float64(state)

function replay()
  global C, model_target, memory
...
function episode!(env::WebEnv, π = RandomPolicy())
  a = 0
  r = 0
  total = 0
  while true
